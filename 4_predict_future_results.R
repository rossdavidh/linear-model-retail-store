...
suppressWarnings(new_data$predictions          <- predict(model, new_data))
cor(new_data$total_sales,new_data$predictions)^2
write.csv(new_data,'./csvs/predictions_future.csv', row.names=FALSE)
'log transform'
model = readRDS(file = "./models/lm_log.rds")
suppressWarnings(new_data$predictions          <- exp(predict(model, new_data)))
cor(new_data$total_sales,new_data$predictions)^2
'exp transform'
model = readRDS(file = "./models/lm_exp.rds")
suppressWarnings(new_data$predictions          <- log(predict(model, new_data)))
cor(new_data$total_sales,new_data$predictions)^2
'square transform'
model = readRDS(file = "./models/lm_pw2.rds")
suppressWarnings(new_data$predictions          <- (predict(model, new_data)))
new_data$predictions[new_data$predictions < 0] <- 0
new_data$predictions                           <- (new_data$predictions)^0.5
cor(new_data$total_sales,new_data$predictions)^2
'square root transform'
model = readRDS(file = "./models/lm_sqr.rds")
suppressWarnings(new_data$predictions          <- (predict(model, new_data))^2)
cor(new_data$total_sales,new_data$predictions)^2
...
suppressWarnings(new_data$predictions          <- (predict(model, new_data))^-1)
cor(new_data$total_sales,new_data$predictions)^2
...
suppressWarnings(new_data$predictions          <- (predict(model, new_data))^-0.5)
new_data$predictions                           <- (new_data$predictions)^-0.5
cor(new_data$total_sales,new_data$predictions)^2
